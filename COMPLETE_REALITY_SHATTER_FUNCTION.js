function animateCosmicStarTimeline(cutsceneEl, aura) {
    // NEW CONCEPT: Reality Shatter - Screen fractures into dimensional shards
    cutsceneState.realityShards = [];
    cutsceneState.sacredGeometry = [];
    cutsceneState.quantumParticles = [];
    cutsceneState.voidRift = { size: 0, opacity: 0, rotation: 0 };
    
    const centerX = cutsceneState.canvas.width / 2;
    const centerY = cutsceneState.canvas.height / 2;
    
    // Create reality shards (triangular fragments of reality)
    const shardCount = 40;
    for (let i = 0; i < shardCount; i++) {
        const angle = (Math.PI * 2 / shardCount) * i;
        const distance = 150 + Math.random() * 200;
        cutsceneState.realityShards.push({
            x: centerX,
            y: centerY,
            targetX: centerX + Math.cos(angle) * distance * 3,
            targetY: centerY + Math.sin(angle) * distance * 3,
            size: 40 + Math.random() * 60,
            rotation: Math.random() * Math.PI * 2,
            rotationSpeed: (Math.random() - 0.5) * 0.1,
            opacity: 0,
            depth: Math.random(),
            color: cutsceneState.currentColor,
            glitch: 0
        });
    }
    
    // Create sacred geometry nodes (for the reassembly)
    const geometryPoints = 12;
    for (let i = 0; i < geometryPoints; i++) {
        const angle = (Math.PI * 2 / geometryPoints) * i;
        const radius = 180;
        cutsceneState.sacredGeometry.push({
